...
int FileFinder::extractFile(uint32_t& fileId)
{
	HANDLE hStorage = NULL;
	HANDLE hFile = NULL;
	HANDLE handle = NULL;
	...
	if (dwErrCode == ERROR_SUCCESS)
	{
		if (!CascOpenFile(hStorage, CASC_FILE_DATA_ID(fileId), 0, CASC_OPEN_BY_FILEID, &hFile))
			dwErrCode = GetLastError();
	...
	if (dwErrCode == ERROR_SUCCESS)
	{
		...
		handle = CreateFile("test", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 0, NULL);
		if (handle == INVALID_HANDLE_VALUE)
			dwErrCode = GetLastError();
	}
